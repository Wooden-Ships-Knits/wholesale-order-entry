...
Both live in `.env.dev`. They are what make it safe to point a dev environment
at live customer data, and **both must stay blank in production `.env`**.

### `MAIL_REDIRECT_TO`

Every outbound message goes to this address instead of its real recipients:
...
a redirected email can never be mistaken for a real one. The Cc is dropped
entirely rather than redirected, so a rep on Cc receives nothing.
